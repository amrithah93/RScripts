attach(depression_cov_dx_symp_rest_conc_matched_qc) #modified dep dataset with first row removed to account for first missing subject in initial ICA analyses
library(corrplot)
...
#test a GLM model with GSC_A_49
Y1 <- Depression_Loadings #list all loading parameters for patients
Y1 <-data.matrix(Y1)
model1 <- aov(Y1 ~ age + sex_coded+ GSC_A_49_Having_a_marked_decreased_interest_in_important_activities, data=depression_cov_dx_symp_rest_conc_matched_qc, na.action=na.omit)
summary.aov(model1) #view model output

#now test all 9 GSC models
Y2 <-Depression_Loadings
Y2 <-data.matrix(Y2)
model2 <-aov(Y2 ~ age + sex_coded + GSC_A_1_Feeling_depressed_or_being_in_a_sad_mood +
               GSC_A_2_Having_a_decreased_interest_in_things_that_are_usually_fun_including_sex +
               GSC_A_3_Experiencing_a_significant_change_in_weight_or_appetite_increased_or_decreased+
               GSC_A_4_Having_recurrent_thoughts_of_death_or_suicide + GSC_A_5_Experiencing_sleep_changes_such_as_a_lack_of_sleep_or_a_marked_increase_in_sleep +
               GSC_A_6_Feeling_physically_agitated_or_being_slowed_down + GSC_A_7_Having_feelings_of_low_energy_or_tiredness+
               GSC_A_8_Having_feelings_of_worthlessness_helplessness_hopelessness_or_guilt + GSC_A_9_Experiencing_decreased_concentration_or_memory)
summary.aov(model2)
